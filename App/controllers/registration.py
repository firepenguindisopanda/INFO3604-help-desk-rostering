...
def create_registration_request(
    ...
    try:
        ...
        if isinstance(profile_picture_url, list):
            profile_picture_url = (
                profile_picture_url[0] if profile_picture_url else None
            )
        ...
        if profile_picture_url:
            if (
                isinstance(profile_picture_url, FileStorage)
                and profile_picture_url.filename
            ):
                ...
                try:
                    ...
                    if start_time and end_time:
                        availability = RegistrationAvailability(
                            registration.id, day, start_time, end_time
                        )
...
def _allowed_transcript_roots(base_path):
    ...
    if base_path:
        roots.extend(
            [
                os.path.normpath(
                    os.path.join(base_path, "..", "uploads", "transcripts")
                ),
...
def resolve_transcript_asset(registration, base_path=None):
    ...
    for candidate in candidates:
        if os.path.exists(candidate) and any(
            _is_within_path(candidate, root) for root in allowed_roots
        ):
            return {
                "mode": "local",
...
def approve_registration(request_id, admin_username):
    ...
    try:
        ...
        if User.query.get(username):
            return (
                False,
                f"A user with username {username} already exists in the system",
            )
        ...
        if stored_password_hash:
            student.password = (
                stored_password_hash  # override hashed password produced in constructor
            )
        else:
        ...
        # Course capabilities
        registration_courses = RegistrationCourse.query.filter_by(
            registration_id=request_id
        ).all()
        for rc in registration_courses:
            db.session.add(
                CourseCapability(
                    assistant_username=username, course_code=rc.course_code
                )
            )
        ...
        reg_availability = RegistrationAvailability.query.filter_by(
            registration_id=request_id
        ).all()
        for reg_avail in reg_availability:
            create_availability(
                username,
                reg_avail.day_of_week,
                reg_avail.start_time,
                reg_avail.end_time,
            )
...
def get_all_registration_requests():
    ...
    # Get registration requests without joinedload for now to avoid relationship issues
    pending = (
        RegistrationRequest.query.filter_by(status="PENDING")
        .order_by(RegistrationRequest.created_at.desc())
        .all()
    )
    approved = (
        RegistrationRequest.query.filter_by(status="APPROVED")
        .order_by(RegistrationRequest.processed_at.desc())
        .all()
    )
    rejected = (
        RegistrationRequest.query.filter_by(status="REJECTED")
        .order_by(RegistrationRequest.processed_at.desc())
        .all()
    )
...
def get_registration_request_by_username(username):
    ...
    return (
        RegistrationRequest.query.filter_by(username=username)
        .order_by(RegistrationRequest.created_at.desc())
        .first()
    )
