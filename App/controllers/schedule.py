...
# Weasyprint will be imported conditionally in functions that need it
...
def get_published_schedules():
    ...
    try:
        return (
            Schedule.query.filter_by(is_published=True)
...
def get_shifts_for_student(username, limit=None):
    ...
    try:
        ...
        query = (
            ...
            .filter(Allocation.username == username, Shift.date >= now.date())
...
def get_shifts_for_student_in_range(username, start_date, end_date):
    ...
    try:
        ...
        logger.error(
            f"Error fetching shifts for student {username} in range {start_date}-{end_date}: {e}"
        )
        return []
...
def check_scheduling_feasibility():
    ...
    try:
        ...
        for assistant in assistants:
            availability_count = Availability.query.filter_by(
        ...
        for assistant in assistants:
            capability_count = CourseCapability.query.filter_by(
        ...
        return {"feasible": False, "message": f"Error checking feasibility: {str(e)}"}


def generate_help_desk_schedule_ortools(
    start_date=None, end_date=None, **generation_options
):
    """
    ...
    with database_transaction_context("help_desk_schedule_generation"):
        ...
        if start_date is None:
            start_date = trinidad_now().replace(
        ...
        generation_payload = {
            k: v for k, v in generation_options.items() if v is not None
        }
        ...
        config_shift_delta = (
            timedelta(minutes=active_config.shift_duration_minutes)
            if active_config
                    ...
                    for hour in range(9, 17):  # 9am through 4pm
                        ...
                        for course in all_courses:
                            try:
                                ...
                                logger.error(
                                    f"Error adding course demand for {course.code} to shift {shift.id}: {str(e)}"
                                )
        ...
        if EAGER_LOADING_AVAILABLE and selectinload:
            assistants_query = assistants_query.options(
                ...
                selectinload(HelpDeskAssistant.student).selectinload(
        ...
        logger.info(
            f"Found {len(assistants)} active assistants with eager-loaded capabilities and availability"
        )
        ...
        logger.info(
            f"Generating schedule with {I} assistants, {J} shifts, and {K} courses"
        )
        ...
        for i in range(I):
            ...
            can_help_with = set(
                cap.course_code for cap in assistant.course_capabilities
            )
        ...
        for i in range(I):
            ...
            for j in range(J):
                ...
                student_availabilities = (
                    assistant.student.availabilities if assistant.student else []
                )
                availabilities = [
                ...
                for avail in availabilities:
                    if (
                        avail.start_time <= shift_start_time
                        and avail.end_time >= shift_end_time
                    ):
                        is_available = True
                        break
        ...
        for j in range(J):
            ...
            logger.debug(
                f"Shift {j} (ID: {shift.id}) has {len(demands)} course demands"
            )
        ...
        for j in range(J):
            for k in range(K):
                ...
                for i in range(I):
                    if (
                        t[i, k] == 1
                    ):  # Only consider if this tutor can teach this course
                        assigned_tutors_for_course.append(x[i, j])

                if (
                    assigned_tutors_for_course
                ):  # Only add shortfall if any tutor can teach this course
        ...
        for j in range(J):
            for k in range(K):
                ...
                if (
                    tutors_for_course_in_shift
                ):  # Only add constraint if any tutor can teach
        ...
        problem_stats = {
            ...
            "courses": K,
            "total_shift_slots": I
            * J,  # theoretical maximum if everyone worked every shift
            "total_course_capacity_needed": sum(
                d[j, k] for j in range(J) for k in range(K)
            ),
            "total_availability": sum(a[i, j] for i in range(I) for j in range(J)),
            "coverage_ratio": sum(a[i, j] for i in range(I) for j in range(J)) / (I * J)
            if I * J > 0
            else 0,
        }
        ...
        if status == cp_model.OPTIMAL or status == cp_model.FEASIBLE:
                ...
                for j in range(J):
                    if solver.Value(x[i, j]) == 1:
                        ...
                        allocation = Allocation(
                            assistant.username, shift.id, schedule.id
                        )
                        new_allocations.append(allocation)
                        assignment_count += 1

                        logger.debug(
                            f"Assigned {assistant.username} to shift {shift.id}"
                        )
            ...
            logger.info(
                f"Schedule generated successfully with status: {status}, {assignment_count} assignments"
            )
...
def generate_help_desk_schedule(start_date=None, end_date=None, **generation_options):
    ...
    return SchedulerFactory.generate_helpdesk_schedule(
        start_date=start_date, end_date=end_date, solver=solver, **generation_options
    )
...
def add_course_demand_to_shift(shift_id, course_code, tutors_required=2, weight=None):
    ...
    db.session.execute(
        text(
            "INSERT INTO shift_course_demand (shift_id, course_code, tutors_required, weight) VALUES (:shift_id, :course_code, :tutors_required, :weight)"
        ),
        {
            "shift_id": shift_id,
...
def get_course_demands_for_shift(shift_id):
    try:
        result = db.session.execute(
            text(
                "SELECT course_code, tutors_required, weight FROM shift_course_demand WHERE shift_id = :shift_id"
            ),
            {"shift_id": shift_id},
        )
        ...
        for row in result:
            demands.append(
                {"course_code": row[0], "tutors_required": row[1], "weight": row[2]}
            )
...
def sync_schedule_data():
    try:
        ...
        logger.info(
            f"Schedule {schedule.id} has {len(shifts)} shifts and {len(allocations)} allocations"
        )

        # Count allocations per shift
        allocation_counts = {}
        for allocation in allocations:
            allocation_counts[allocation.shift_id] = (
                allocation_counts.get(allocation.shift_id, 0) + 1
            )

        shifts_without_allocations = [
            shift.id for shift in shifts if allocation_counts.get(shift.id, 0) == 0
        ]
        if shifts_without_allocations:
            logger.warning(
                f"Found {len(shifts_without_allocations)} shifts without allocations"
            )
        ...
        for allocation in allocations:
            ...
            if not student:
                logger.warning(
                    f"Allocation {allocation.id} references non-existent student {allocation.username}"
                )
...
def publish_and_notify(schedule_id):
    try:
        ...
        if not sync_success:
            logger.warning(
                "Schedule published but data sync failed or was not necessary"
            )
...
def clear_schedule_by_id(schedule_id):
    try:
        ...
        if shift_ids:
            ...
            db.session.execute(
                text(
                    f"DELETE FROM shift_course_demand WHERE shift_id IN ({shift_ids_str})"
                )
            )
        ...
        logger.info(
            f"Schedule {schedule_id} cleared successfully: {shift_count} shifts and {allocation_count} allocations removed"
        )
...
def clear_schedule():
    try:
        ...
        if not schedule:
            return {"status": "success", "message": "No schedule exists to clear"}
        ...
        logger.info(
            f"Schedule cleared successfully: {shift_count} shifts and {allocation_count} allocations removed"
        )
...
def get_schedule_data(schedule_id):
    ...
    try:
        ...
        if EAGER_LOADING_AVAILABLE and selectinload:
            logger.info(
                f"Loaded schedule {schedule_id} with {len(schedule.shifts)} shifts using eager loading"
            )
        else:
            logger.info(
                f"Loaded schedule {schedule_id} with {len(schedule.shifts)} shifts (eager loading not available)"
            )
        ...
        logger.info(
            f"Successfully formatted schedule data for {len(formatted_schedule['days'])} days"
        )
...
def get_current_schedule():
    ...
    try:
        ...
        logger.info(
            f"Loaded schedule {schedule.id} with {len(schedule.shifts)} shifts using eager loading"
        )
        ...
        for day_idx in range(5):  # Monday to Friday
            day_date = (
                schedule.start_date + timedelta(days=day_idx)
...
def generate_lab_schedule_ortools(start_date=None, end_date=None, **generation_options):
    ...
    try:
        ...
        if I == 0:
            return {
                "status": "error",
                "message": "No active lab assistants available for scheduling",
            }
        ...
        while current_date <= end_date:
            ...
            if current_date.weekday() < 6:  # 0=Monday through 5=Saturday
                ...
                for hour in range(8, 17, 4):  # 8am, 12pm, 4pm
                    ...
                    shift = create_shift(
                        current_date, shift_start, shift_end, schedule.id
                    )
                    shifts.append(shift)
        ...
        if J == 0:
            return {
                "status": "error",
                "message": "No shifts were created for the selected date range",
            }
        ...
        for i in range(I):
            ...
            for j in range(J):
                ...
                availabilities = Availability.query.filter_by(
                    username=assistant.username, day_of_week=day_of_week
                ).all()
        ...
        solver.parameters.max_time_in_seconds = (
            60.0  # Increase time limit to 60 seconds
        )
        solver.parameters.num_search_workers = 8  # Use more worker threads
        solver.parameters.log_search_progress = True  # Log search progress
        status = solver.Solve(model)
...
def generate_help_desk_schedule_pdf(schedule_data):
    ...
    try:
        ...
        html_content = render_template(
            "admin/schedule/pdf_template.html", schedule=schedule_data
        )
...
def generate_schedule_pdf(schedule_data, export_format="standard"):
    if not WEASYPRINT_AVAILABLE:
        raise ImportError(
            "WeasyPrint is not available on this system. PDF generation is not supported."
        )
    """
    ...
    try:
        ...
        logger.info(
            f"Successfully generated PDF for {schedule_type} schedule (format: {export_format})"
        )
        return pdf_buffer
...
def get_schedule_summary_stats(schedule_type):
    ...
    try:
        ...
        coverage_percentage = (
            (assigned_shifts / total_shifts * 100) if total_shifts > 0 else 0.0
        )
    ...
    try:
        ...
        for entry in assignments:
            ...
            if day_index is None:
                logger.warning(
                    f"Skipping assignment with invalid day label: {day_label}"
                )
                continue
            ...
            if start_hour is None:
                logger.warning(
                    f"Skipping assignment with invalid time slot: {time_slot}"
                )
                continue

            shift_start = datetime.combine(
                shift_date.date(), datetime.min.time()
            ) + timedelta(hours=start_hour)
            shift_end_hour = _calculate_shift_end(schedule_type, start_hour)
            shift_end = datetime.combine(
                shift_date.date(), datetime.min.time()
            ) + timedelta(hours=shift_end_hour)

            shift = Shift.query.filter_by(
                schedule_id=schedule.id, date=shift_date, start_time=shift_start
            ).first()
            ...
            for staff in staff_members:
                ...
                if not student:
                    logger.warning(
                        f"Skipping allocation for missing student id {staff_id}"
                    )
                    continue
                existing_allocation = Allocation.query.filter_by(
                    shift_id=shift.id, username=student.username
                ).first()
    ...
    try:
        ...
        if shift_id:
            target_shift = Shift.query.filter_by(
                id=shift_id, schedule_id=schedule.id
            ).first()
        else:
            day_index = _normalize_day_index(day_label)
            if day_index is None:
        ...
        allocation = Allocation.query.filter_by(
            shift_id=target_shift.id, username=staff_id
        ).first()
        if not allocation:
            return {
                "status": "error",
                "message": "Staff assignment not found for the specified shift.",
            }, 404
...
def _check_time_slot_availability(availability_slots, requested_time):
    ...
    for slot in availability_slots:
        try:
            avail_start_time = _normalize_time_object(
                slot.start_time, "availability start_time"
            )
            avail_end_time = _normalize_time_object(
...
def list_available_staff_for_slot(
    schedule_type: str, day_label: str, time_slot: Union[str, int]
):
    try:
        day_index = _normalize_day_index(day_label)
        if day_index is None:
        ...
        for assistant in assistants:
            availability_slots = Availability.query.filter_by(
                username=assistant.username, day_of_week=day_index
            ).all()
            ...
            matching_slot = _check_time_slot_availability(
                availability_slots, requested_time
            )
            if matching_slot:
...
def check_staff_availability_for_slot(
    schedule_type: str,
    staff_id: Union[str, int],
    day_label: str,
    time_slot: Union[str, int],
):
    try:
        ...
        availability_slots = Availability.query.filter_by(
            username=staff.username, day_of_week=day_index
        ).all()
        matching_slot = _check_time_slot_availability(
            availability_slots, requested_time
        )
        ...
        existing_assignment = (
            ...
            .filter(
                ...
                Shift.schedule_id == schedule_id,
                db.extract("dow", Shift.date)
                == day_index,  # Use extract for day of week comparison
                db.extract("hour", Shift.start_time)
                == hour,  # Use extract for hour comparison
            )
            .first()
        )
...
def batch_staff_availability(schedule_type: str, queries: list[dict[str, Any]]):
    ...
    try:
        ...
        for query in queries:
            ...
            result, status_code = check_staff_availability_for_slot(
                schedule_type, staff_id, day, time_slot
            )
...
def batch_list_available_staff_for_slots(schedule_type: str, slot_queries: list[dict]):
    ...
    try:
        ...
        for assistant in assistants:
            if (
                getattr(assistant, "student", None)
                and getattr(assistant.student, "availabilities", None) is not None
            ):
                # Filter only relevant day indices
                avail_map[assistant.username] = [
                    av
                    for av in assistant.student.availabilities
                    if av.day_of_week in day_indices
                ]
        ...
        if missing_usernames and day_indices:
            extra_avails = Availability.query.filter(
                Availability.username.in_(missing_usernames),
                Availability.day_of_week.in_(list(day_indices)),
            ).all()
            ...
            for assistant in assistants:
                ...
                if match:
                    assistant_data = {
                        ...
                        or (
                            assistant.student.get_name()
                            if getattr(assistant, "student", None)
                            else None
                        ),
                    }
                    available_staff.append(assistant_data)
    ...
    try:
        ...
        if not schedule:
            return (
                None,
                None,
                {"status": "error", "message": _ERROR_SCHEDULE_NOT_FOUND},
                404,
            )
        ...
        if not schedule_data:
            return (
                None,
                None,
                {"status": "error", "message": "Unable to generate schedule data."},
                500,
            )
        ...
        filename = (
            f"{schedule_type}_schedule_{datetime.now().strftime('%Y%m%d_%H%M%S')}.pdf"
        )
