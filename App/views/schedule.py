...
def _format_time_label(hour: int, minute: int) -> str:
    ...
    try:
        display_dt = datetime.now().replace(
...
def schedule():
    ...
    try:
        ...
        current_app.logger.warning(
            "Unable to load active schedule configuration: %s", exc
        )
...
def save_schedule():
    ...
    try:
        ...
        result, status_code = save_schedule_assignments(
            schedule_type, start_date_str, end_date_str, assignments
        )
...
def remove_staff_from_shift():
    ...
    try:
        ...
        result, status_code = remove_staff_allocation(
            schedule_type, staff_id, cell_day, cell_time, shift_id
        )
        return jsonify(result), status_code
...
def generate_schedule_endpoint():
    ...
    try:
        ...
        current_app.logger.info(
            "Generating %s schedule from %s to %s", user.role, start_date, end_date
        )
        ...
        current_app.logger.warning(
            "Client disconnected during schedule generation: %s", e
        )
        abort(499)
    except Exception as e:
        import traceback
...
@schedule_views.route(
    "/api/schedule/<int:schedule_id>/publish_with_sync", methods=["POST"]
)
@jwt_required()
@admin_required
...
def get_current_schedule_endpoint():
    ...
    try:
        ...
        current_app.logger.info(
            "Getting current %s schedule (ID: %s)", schedule_type, schedule_id
        )
        ...
        if not schedule:
            current_app.logger.info(
                "No %s schedule found with ID %s", schedule_type, schedule_id
            )
            return jsonify(
                {"status": "error", "message": f"No {schedule_type} schedule found"}
            ), 404
        ...
        current_app.logger.info(
            "Loaded %s shifts with eager loading", len(schedule.shifts)
        )
        ...
        if schedule_type == "helpdesk":
            try:
                ...
                current_app.logger.warning(
                    "Failed to load active schedule configuration for API response: %s",
                    exc,
                )
        ...
        if schedule_type == "lab":
            ...
            for day_idx in range(6):
                ...
                for config in lab_shifts_config:
                    ...
                    matching_shift = next(
                        (s for s in actual_shifts_today if s["hour"] == hour), None
                    )
        ...
        # HELPDESK SCHEDULE LOGIC
            ...
            configured_days = {
                idx
                for idx in range(5)
                if _is_configured_helpdesk_day(idx, active_config)
            }
            ...
            for day_idx in range(5):
                ...
                for slot in time_slots:
                    ...
                    matching_shift = next(
                        (
                            s
                            for s in actual_shifts_today
                            if s.get("start_minutes") == slot_minutes
                        ),
                        None,
                    )
        ...
        try:
            batch_result, batch_status = batch_list_available_staff_for_slots(
                schedule_type, slot_queries
            )
        ...
        current_app.logger.warning(
            "Client disconnected while getting current schedule: %s", e
        )
...
def get_available_staff():
    ...
    try:
        ...
        if not day:
            return jsonify(
                {"status": "error", "message": "day parameter is required"}
            ), 400

        if not time_slot:
            return jsonify(
                {"status": "error", "message": "time parameter is required"}
            ), 400
        ...
        # Delegate to controller
        result, status_code = list_available_staff_for_slot(
            schedule_type, day, time_slot
        )

        if status_code == 200 and result.get("status") == "success":
            return jsonify(
                {"status": "success", "staff": result.get("available_staff", [])}
            )
        else:
            return jsonify(result), status_code
...
def check_staff_availability():
    ...
    try:
        ...
        if not staff_id:
            return jsonify(
                {"status": "error", "message": "staff_id parameter is required"}
            ), 400
...
def download_schedule_pdf():
    ...
    try:
        ...
        # Delegate to controller
        pdf_buffer, filename, error_response, status_code = (
            generate_schedule_pdf_for_type(schedule_type)
        )
