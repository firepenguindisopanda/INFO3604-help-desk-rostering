...
def _format_registration_data(registration_data):
    ...
    def _serialize_registrations(registrations):
        """Convert RegistrationRequest objects to dictionaries"""
        if not registrations:
            return []

        serialized = []
        for reg in registrations:
            if hasattr(reg, "get_json"):
                serialized.append(reg.get_json())
            elif hasattr(reg, "to_dict"):
                serialized.append(reg.to_dict())
            elif isinstance(reg, dict):
                serialized.append(reg)
            else:
                # Fallback: try to convert to dict manually
                try:
                    serialized.append(vars(reg))
                except Exception:
    ...
    return {
        "pending": _serialize_registrations(registration_data.get("pending", [])),
        "approved": _serialize_registrations(registration_data.get("approved", [])),
        "rejected": _serialize_registrations(registration_data.get("rejected", [])),
    }
...
def get_registrations_api():
    ...
    try:
        ...
        return api_error(f"{FAILED_TO_RETRIEVE_MSG}: {str(e)}", status_code=500)
...
def approve_registration_api(registration_id):
    ...
    try:
        ...
        return api_error(f"{FAILED_TO_APPROVE_MSG}: {str(e)}", status_code=500)
...
def reject_registration_api(registration_id):
    ...
    try:
        ...
        if reason and len(reason) > 500:
            return api_error(
                "Rejection reason must be 500 characters or less", status_code=400
            )
        ...
        return api_error(f"{FAILED_TO_REJECT_MSG}: {str(e)}", status_code=500)
...
def get_registration_details_api(registration_id):
    ...
    try:
        ...
        if transcript_asset:
            ...
            registration_data["transcript_type"] = transcript_asset.get(
                "mode", "unknown"
            )
        else:
        ...
        return api_error(
            f"Failed to retrieve registration details: {str(e)}", status_code=500
        )
...
def get_transcript_info_api(registration_id):
    ...
    try:
        ...
        return api_error(f"Failed to get transcript info: {str(e)}", status_code=500)


@api_v2.route(
    "/registrations/<int:registration_id>/transcript/download", methods=["GET"]
)
@jwt_required_secure()
@admin_required
def download_transcript_api(registration_id):
    ...
    try:
        ...
        return api_error(f"{FAILED_TO_DOWNLOAD_MSG}: {str(e)}", status_code=500)
...
def get_pending_registrations_api():
    ...
    try:
        ...
        return api_error(
            f"{FAILED_TO_RETRIEVE_MSG} pending registrations: {str(e)}", status_code=500
        )
