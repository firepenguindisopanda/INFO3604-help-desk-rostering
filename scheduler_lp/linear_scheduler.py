...
class SchedulerConfig:
    ...
    understaffed_penalty: float = 25.0
...
def solve_helpdesk_schedule(
    ...
    *,
    ...
    """Solve the scheduling problem using PuLP."""
    ...
    min_hours_vars, max_hours_vars = _build_hour_slack_variables(assistants)
...
def _build_hour_slack_variables(
    assistants: Sequence[Assistant],
) -> Tuple[Dict[str, pulp.LpVariable], Dict[str, pulp.LpVariable]]:
    min_hours_vars: Dict[str, pulp.LpVariable] = {}
    max_hours_vars: Dict[str, pulp.LpVariable] = {}
    for assistant in assistants:
        if assistant.min_hours > 0:
            min_hours_vars[assistant.id] = pulp.LpVariable(f"min_hours_shortfall_{assistant.id}", lowBound=0)
    ...
    return min_hours_vars, max_hours_vars
...
def _build_hour_constraints(
    ...
    for assistant in assistants:
        ...
        if assistant.min_hours > 0:
            slack = min_hours_vars[assistant.id]
            min_constraint = total_hours + slack >= assistant.min_hours
            min_constraint.name = f"min_hours_{assistant.id}"
            yield min_constraint
        if assistant.max_hours is not None:
            excess = max_hours_vars[assistant.id]
